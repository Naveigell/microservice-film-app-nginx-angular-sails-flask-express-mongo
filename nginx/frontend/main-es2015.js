...
let AppComponent = class AppComponent {
    ...
    saveFilm() {
        const { title, description } = Object.assign({}, this.film.insert.data);
        this.http.post(`${this.gateway}/films`, { title, description }).subscribe(data => {
    ...
    retrieveFilms() {
        this.http.get(`${this.gateway}/films`).subscribe(data => {
    ...
    retrieveDirectors() {
        this.http.get(`${this.gateway}/directors`).subscribe(data => {
    ...
    removeDirector(index) {
        const { _id = '' } = Object.assign({}, this.directors[index]);
        const httpOptions = {
            headers: new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpHeaders"]({ 'Content-Type': 'application/json' }), body: { id: _id }
        };
        this.http.delete(`${this.gateway}/directors`, httpOptions).subscribe(data => {
            if (data["message"]) {
                this.retrieveDirectors();
            }
            else if (data["error"]) {
    ...
    removeFilms(index) {
        const { _id = '' } = Object.assign({}, this.films[index]);
        const httpOptions = {
            headers: new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpHeaders"]({ 'Content-Type': 'application/json' }), body: { id: _id }
        };
        this.http.delete(`${this.gateway}/films`, httpOptions).subscribe(data => {
            if (data["message"]) {
                this.retrieveFilms();
            }
            else if (data["error"]) {
    ...
    updateFilm(film) {
        const { _id = '', title = '', description = '' } = Object.assign({}, film);
        this.http.put(`${this.gateway}/films`, { id: _id, title, description }).subscribe(data => {
            if (data["message"]) {
                this.retrieveFilms();
            }
            else if (data["error"]) {
    ...
    updateDirector(director) {
        const { _id = '', name = '', born = '' } = Object.assign({}, director);
        this.http.put(`${this.gateway}/directors`, { id: _id, name, born }).subscribe(data => {
            if (data["message"]) {
                this.retrieveDirectors();
            }
            else if (data["error"]) {
    ...
    saveDirector() {
        const { name = '', born = '' } = Object.assign({}, this.director.insert.data);
        this.http.post(`${this.gateway}/directors`, { name, born }).subscribe(data => {
...
module.exports = __webpack_require__(/*! D:\Application\Project\Microservices\film-app-without-database\frontend\src\main.ts */"./src/main.ts");
